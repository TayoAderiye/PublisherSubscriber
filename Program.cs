...
//Publish message
app.MapPost("api/topics/{id}/message", async (AppDbContext context, int id, Message message) => {
...
app.MapGet("api/subscriptions/{id}/messages", async (AppDbContext context, int id) => {
    bool subs = await context.Subscriptions.AnyAsync(x => x.Id == id);
    if(!subs)
...
app.MapPost("api/subscriptions/{id}/messages", async (AppDbContext context, int id, int[] confs) => {
    ...
    return Results.Ok($"Acknowledged {count}/{confs.Length} messgages");
});
