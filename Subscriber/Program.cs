...
static async Task<List<int>> GetMessagesAsync(HttpClient httpClient)
{
    ...
    try
    {
        newMessages = await httpClient.GetFromJsonAsync<List<MessageReadDto>>("https://localhost:5267/api/subscriptions/2/messages");
    }
    catch
    {
        return ackIds;
...
{
    var response = await httpClient.PostAsJsonAsync("https://localhost:5267/api/subscriptions/2/messages", ackIds);
